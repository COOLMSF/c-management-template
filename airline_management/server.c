...
int tail_add_list_user(struct list_node *u_head,int connfd)
{
    ...
    printf("初始账户余额:");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"初始账户余额:\n");
    send(connfd,sendbuf,strlen(sendbuf),0);
    while(1)
        if(char_input(connfd) == 1)
        {
            strcpy(Node->money_left,chars);
            break;
        }
    ...
    // 将余额也写入进文件
    sprintf(buff,"%s,%s,%s",Node->name,Node->passwd, Node->money_left);
...
struct list_node *user_query_fun(struct list_node *u_head,struct list_node *p_head, int connfd)
{    
    ...
    //     // 找到用户，输出其余额
    //     if(strcmp(u_head->name,tmp->name) == 0) {
    //         printf("你的余额为%d\n", atoi(tmp->money_left));
    //         bzero(sendbuf,sizeof(sendbuf));
    //         sprintf(sendbuf, "你的余额为%d\n", atoi(u_head->money_left));
    //         send(connfd,sendbuf,strlen(sendbuf),0);
    //     }
    // }
...
struct list_node *user_topup_fun(struct list_node *u_head,struct list_node *p_head,  int connfd)
{
    printf("你当前的余额为%d\n", atoi(u_head->money_left));
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf, "你当前的余额为%d\n", atoi(u_head->money_left));
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("请输入你要充值的数量:");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"请输入你要充值的数量:\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    char money_topup_str[10];
    while(1)
        if(char_input(connfd) == 1)
        {
            strcpy(money_topup_str,chars);
            break;
        }

    if (atoi(money_topup_str) < 0) {
        printf("输入金额有误:");
        bzero(sendbuf,sizeof(sendbuf));
        sprintf(sendbuf,"输入金额有误:\n");
        send(connfd,sendbuf,strlen(sendbuf),0);

        return;
    }
    // 修改用户余额
    // strcpy(Node->money_left,atoi(chars);
    int money_left_int = atoi(u_head->money_left);
    int money_topup_int = atoi(chars);
    sprintf(u_head->money_left, "%d", money_left_int + money_topup_int);

    printf("充值成功:");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"充值成功:\n");
    send(connfd,sendbuf,strlen(sendbuf),0);
}

struct list_node *user_modify_fun(struct list_node *u_head,struct list_node *p_head,int connfd)
{

}

struct list_node *user_get_ticket_fun(struct list_node *u_head,struct list_node *p_head,int connfd)
{
    char buff[500],nm[10];
    char user_data[100], user_data_filename[100];
...
struct list_node *user_print_ticket_fun(struct list_node *u_head,struct list_node *p_head,int connfd)
{
    int a,b,c,d;
...
struct list_node *user_communicate_fun(struct list_node *u_head,struct list_node *p_head,int connfd)
{
...
int user_buy_fun(struct list_node *u_head,struct list_node *p_head,int connfd)
{
    ...
    list_for_each_prev(p,&(p_head->list))//查看用户是否购买过此航班
    {
        ...
        int diff = atoi(u_head->money_left) - atoi(tmp->price);
        if (diff < 0) {
            printf("余额不足，请充值 \n");
            bzero(sendbuf,sizeof(sendbuf));
            sprintf(sendbuf,"余额不足，请充值 \n");
            send(connfd,sendbuf,strlen(sendbuf),0);
            return;
        }
        // 写入内存
        sprintf(u_head->money_left, "%d", diff);
    ...
    list_for_each_safe(p,q,&(u_head->list))
        {
            ...
            {    
                    int ret_money;
                    int total_money;
                    char ret_money_str[20];

                    ret_money = (int )(atoi(tmp->price) * 0.9);
                    total_money = atoi(u_head->money_left) + ret_money;
                    sprintf(ret_money_str, "%d", ret_money);
                    sprintf(u_head->money_left, "%d", total_money);
                    printf("退票需要收取百分之10手续费 返回了你%srmb\n",ret_money_str);
                    bzero(sendbuf,sizeof(sendbuf));
                    sprintf(sendbuf,"退票需要收取百分之10手续费 返回了你%srmb\n",ret_money_str);
                    send(connfd,sendbuf,strlen(sendbuf),0);
                    list_del(p); 
                    free(tmp); 
                    return 0;
...
int menu_fun(int connfd)
{
    ...
    printf("7.查询余额\n");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"7.查询余额\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("8.账户充值");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"8.账户充值\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("9.修改信息");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"9.修改信息\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("10.取票服务");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"10.取票服务\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("11.打印发票");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"11.打印发票\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("12.聊天功能");
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"12.聊天功能\n");
    send(connfd,sendbuf,strlen(sendbuf),0);

    printf("13.退出\n");//退出登陆
    bzero(sendbuf,sizeof(sendbuf));
    sprintf(sendbuf,"13.退出\n");
    ...
    {    
        ...
        switch(touch)
        {
                ...
                case 6://6、退票
                    ...
                    break;
                case 7:
                    user_query_fun(u_head,p_head,connfd);
                    break;
                case 8:
                    user_topup_fun(u_head,p_head, connfd);
                    break;
                case 9:
                    user_modify_fun(u_head,p_head,connfd);
                    break;
                case 10:
                    user_get_ticket_fun(u_head,p_head,connfd);
                    break;
                case 11:
                    user_print_ticket_fun(u_head,p_head,connfd);
                    break;
                case 12:
                    user_communicate_fun(u_head,p_head,connfd);
                    break;
                case 13://7、退出登陆
                    return 1;
                    break;
...
{    
    ...
    tmp = list_entry(p,struct list_node,list);
    chdir("./register");
    printf("写入:%s.txt",tmp->name);
    sprintf(user_data_filename,"%s.txt",tmp->name);
    FILE *fp = fopen(user_data_filename,"w");
    if (fp == NULL) {
        perror("fopen");
        exit(EXIT_FAILURE);
    }
    ...
    // 将余额也写入进文件
    sprintf(user_data,"%s,%s,%s",tmp->name,tmp->passwd,tmp->money_left);
    printf("数据为:%s",user_data);
    fwrite(user_data,50,1,fp);
    fclose(fp);
    chdir("../");
    }
